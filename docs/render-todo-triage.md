...
| RND-09 | ✓ Done (gaming-PC sprint) — `D3D10_SVC_OBJECT` in `parseConstants` is unreachable in valid HLSL SM4+: sampler/texture objects are resource-bound via `GetResourceBindingDesc` (handled in `parseResources`), not cbuffer members. `VERIFY` stub + dead DX9 sampler-registration block removed; replaced with a knowledge comment. `bSkip = TRUE` behaviour preserved. |
