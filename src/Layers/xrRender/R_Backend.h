...
public:
	...
	void set_c(R_constant* C, const Fmatrix& A) { if (C) constants.set(C, A); }
	void set_c(R_constant* C, const Fvector4& A) { if (C) constants.set(C, A); }
	void set_c(R_constant* C, float x, float y, float z, float w) { if (C) constants.set(C, x, y, z, w); }
	void set_ca(R_constant* C, u32 e, const Fmatrix& A) { if (C) constants.seta(C, e, A); }
	void set_ca(R_constant* C, u32 e, const Fvector4& A) { if (C) constants.seta(C, e, A); }
	void set_ca(R_constant* C, u32 e, float x, float y, float z, float w)
	{
		if (C) constants.seta(C, e, x, y, z, w);
	}
#if defined(USE_DX10) || defined(USE_DX11)
	void set_c(R_constant* C, float A) { if (C) constants.set(C, A); }
	void set_c(R_constant* C, int A) { if (C) constants.set(C, A); }
#endif	//	USE_DX10


	// constants - LPCSTR (slow)
	void set_c(LPCSTR n, const Fmatrix& A) { if (ctable) set_c(&*ctable->get(n), A); }
	void set_c(LPCSTR n, const Fvector4& A) { if (ctable) set_c(&*ctable->get(n), A); }
	void set_c(LPCSTR n, float x, float y, float z, float w) { if (ctable) set_c(&*ctable->get(n), x, y, z, w); }
	void set_ca(LPCSTR n, u32 e, const Fmatrix& A) { if (ctable) set_ca(&*ctable->get(n), e, A); }
	void set_ca(LPCSTR n, u32 e, const Fvector4& A) { if (ctable) set_ca(&*ctable->get(n), e, A); }
	void set_ca(LPCSTR n, u32 e, float x, float y, float z, float w)
	{
		if (ctable) set_ca(&*ctable->get(n), e, x, y, z, w);
	}
#if defined(USE_DX10) || defined(USE_DX11)
	void set_c(LPCSTR n, float A) { if (ctable) set_c(&*ctable->get(n), A); }
	void set_c(LPCSTR n, int A) { if (ctable) set_c(&*ctable->get(n), A); }
#endif	//	USE_DX10

	// constants - shared_str (average)
	void set_c(shared_str& n, const Fmatrix& A) { if (ctable) set_c(&*ctable->get(n), A); }
	void set_c(shared_str& n, const Fvector4& A) { if (ctable) set_c(&*ctable->get(n), A); }
	void set_c(shared_str& n, float x, float y, float z, float w)
	{
		if (ctable)
		{
			ref_constant foo = ctable->get(n);
			set_c(&*foo, x, y, z, w);
		}
	}

	void set_ca(shared_str& n, u32 e, const Fmatrix& A) { if (ctable) set_ca(&*ctable->get(n), e, A); }
	void set_ca(shared_str& n, u32 e, const Fvector4& A) { if (ctable) set_ca(&*ctable->get(n), e, A); }
	void set_ca(shared_str& n, u32 e, float x, float y, float z, float w)
	{
		if (ctable) set_ca(&*ctable->get(n), e, x, y, z, w);
	}
#if defined(USE_DX10) || defined(USE_DX11)
	void set_c(shared_str& n, float A) { if (ctable) set_c(&*ctable->get(n), A); }
	void set_c(shared_str& n, int A) { if (ctable) set_c(&*ctable->get(n), A); }
#endif	//	USE_DX10
