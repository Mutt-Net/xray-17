...
void CALifeGraphRegistry::setup_current_level()
{
	...
	[[maybe_unused]] int id = pApp->Level_ID(*(*I).second.name(), "1.0", true);
                                 ...
                                 GameGraph::_GRAPH_ID game_vertex_id, bool alife_query, bool remove_children)
{
	...
	{
		CSE_ALifeDynamicObject* obj = smart_cast<CSE_ALifeDynamicObject*>(item);
		VERIFY(obj);
		obj->m_tGraphID = game_vertex_id;
		level().add(obj);
	}
